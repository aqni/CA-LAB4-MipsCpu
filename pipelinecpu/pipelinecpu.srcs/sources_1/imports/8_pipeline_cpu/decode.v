...
//    output     [ 32:0] jbr_bus,     // 跳转总线
    ...
    output     [188:0] ID_EXE_bus,  // ID->EXE总线
...
//-----{分支指令执行}begin
   //bd_pc,分支跳转指令参与计算的为延迟槽指令的PC值，即当前分支指令的PC+4
    ...
    wire [31:0] j_target;
    //MOD
//    assign j_taken = (inst_J | inst_JAL | inst_jr) & ID_over ;
...
//    assign j_bus = {j_taken, j_target};
    assign j_taken = (inst_J | inst_JAL) & ID_over ;
    //寄存器跳转地址为rs_value,其他跳转为{bd_pc[31:28],target,2'b00}
...
//                          inst_imm_sign ?  {{16{imm[15]}}, imm} : rt_value;
    ...
    assign read_rs=~(inst_j_link|inst_shf_sa);
...
//    assign store_data = rt_value;
    assign ID_EXE_bus = {read_rs,read_rt,br_control,br_target, //MOD:REG需用的信息
                         rs,rt,inst_no_rs,inst_no_rt,
                         multiply,mthi,mtlo,                   //EXE需用的信息,新增
