...
//-----{乘法器}end
    ...
    assign wait_store_data= (|store_rf_addr) &
                            ((store_rf_addr==MEM_wdest & ~MEM_wvalid)|(store_rf_addr==WB_wdest & WB_wvalid));
...
//-----{EXE执行完成}begin
    ...
    //但对于乘法操作，需要多拍完成
    //MOD
    assign EXE_over = EXE_valid & (~multiply | mult_end) & ~wait_store_data;
//-----{EXE执行完成}end
...
//-----{EXE->MEM总线}begin
    ...
    assign EXE_MEM_bus = {mem_control,forward_store_data,  //load/store信息和store数据
                          exe_result,                      //exe运算结果
